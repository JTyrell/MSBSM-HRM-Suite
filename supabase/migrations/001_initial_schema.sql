...
CREATE TYPE contract_type AS ENUM ('permanent', 'contract', 'part_time', 'casual', 'temporary', 'intern');
CREATE TYPE role_tier AS ENUM ('ancillary', 'maintenance', 'admin', 'ict_tech', 'ict_sysadmin', 'ict_mgmt', 'faculty', 'executive', 'hr');
...
CREATE TYPE pto_type AS ENUM ('sick', 'vacation', 'personal', 'other', 'bereavement', 'jury_duty', 'fmla');
...
CREATE TYPE announcement_category AS ENUM ('general', 'urgent', 'policy', 'event', 'celebration');
...
CREATE TYPE shift_assignment_status AS ENUM ('scheduled', 'confirmed', 'completed', 'no_show', 'swapped');
...
CREATE TYPE expense_status AS ENUM ('draft', 'submitted', 'approved', 'rejected', 'processing', 'paid');
...
CREATE TYPE kudos_category AS ENUM ('teamwork', 'innovation', 'leadership', 'service', 'dedication', 'growth');
...
CREATE TABLE employees (
  id                UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  employee_id       TEXT NOT NULL UNIQUE,           -- e.g. EMP-0001
  ...
  work_location_id  UUID,       -- FK added after geofences table
  address           TEXT,
  ...
  trn               VARCHAR(9),               -- Tax Registration Number (9 digits)
  nis_number        VARCHAR(12),              -- NIS format: XX-XXXXXX-X
  nht_number        VARCHAR(20),              -- NHT contribution number
  paye_tax_code     paye_code DEFAULT 'A',    -- PAYE code A-E
  ...
  -- ─── Constraints ──────────────────────────────────────────────
  CONSTRAINT chk_trn_format CHECK (trn IS NULL OR trn ~ '^\d{9}$'),
  CONSTRAINT chk_nis_format CHECK (nis_number IS NULL OR nis_number ~ '^\d{2}-\d{6}-\d{1}$'),
...
ALTER TABLE employees
  ADD CONSTRAINT fk_employees_work_location
  FOREIGN KEY (work_location_id) REFERENCES geofences(id);
...
CREATE TABLE schedule_conflicts (
  ...
  conflict_type   TEXT NOT NULL,  -- 'overtime', 'double_booking', 'budget_overrun', 'availability'
...
CREATE TABLE chat_messages (
  ...
  content     TEXT NOT NULL,
  agent_type  TEXT,            -- 'hr_assistant', 'pto_fairy', etc.
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE statutory_rates (
  id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  rate_key        TEXT NOT NULL,         -- 'nis_employee', 'nis_employer', 'nht_employee', etc.
  rate_value      NUMERIC(8,4) NOT NULL, -- e.g. 0.03 for 3%
  ceiling_value   NUMERIC(14,2),         -- e.g. 32400 for weekly NIS ceiling
  ceiling_period  TEXT,                  -- 'weekly', 'monthly', 'annual'
  description     TEXT,
  effective_from  DATE NOT NULL,
  expires_on      DATE,                  -- NULL = current/no expiry
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE channels (
  ...
  name          TEXT NOT NULL,
  description   TEXT,
  type          TEXT NOT NULL DEFAULT 'department',  -- 'department', 'private', 'group', 'announcement'
  department_id UUID REFERENCES departments(id),
  company_id    UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
...
CREATE TABLE expense_claims (
  ...
  currency      TEXT NOT NULL DEFAULT 'JMD',
  category      TEXT NOT NULL,            -- 'travel', 'meals', 'supplies', 'equipment', 'training', 'other'
  receipt_url   TEXT,
...
CREATE TABLE benefits (
  ...
  name          TEXT NOT NULL,
  description   TEXT,
  category      TEXT NOT NULL,            -- 'health', 'dental', 'vision', 'life', 'retirement', 'wellness'
  provider      TEXT,
...
CREATE TABLE benefit_enrollments (
  ...
  coverage_type TEXT NOT NULL DEFAULT 'individual',  -- 'individual', 'family', 'individual_plus_one'
...
CREATE TABLE courses (
  ...
  title         TEXT NOT NULL,
  description   TEXT,
  category      TEXT NOT NULL,             -- 'compliance', 'technical', 'leadership', 'safety', 'onboarding'
...
CREATE TABLE documents (
  ...
  file_size     BIGINT,
  category      TEXT NOT NULL DEFAULT 'general',  -- 'policy', 'contract', 'certification', 'personal', 'general'
...
CREATE TABLE feedback_entries (
  ...
  type        TEXT NOT NULL DEFAULT 'survey',   -- 'survey', 'peer', 'manager', 'self'
  rating      INTEGER CHECK (rating >= 1 AND rating <= 5),
  content     TEXT,
  is_anonymous BOOLEAN NOT NULL DEFAULT false,
  responses   JSONB,                            -- Survey question responses
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE wellness_entries (
  id            UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  employee_id   UUID NOT NULL REFERENCES employees(id) ON DELETE CASCADE,
  type          TEXT NOT NULL,              -- 'mood', 'activity', 'check_in'
  mood_score    INTEGER CHECK (mood_score >= 1 AND mood_score <= 5),
  activity_type TEXT,                       -- 'steps', 'exercise', 'meditation', 'break'
  value         NUMERIC(10,2),              -- e.g. step count, minutes
  note          TEXT,
